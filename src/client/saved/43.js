...
function rotatePosition (positionOld, axis, angle, offset, translate) {
  ...
  return [
    result[0] + translate.x,
    result[1] + translate.y,
    result[2],
  ]
}

const stops = [20, 40, 50, 55]
...
//     createCercle(n)
//   } else if (n < stops[1]) {
//     createCercle(n, 45, { x: 0, y: stops[0] * 2, z: 0 }, { x: 0, y: 0, z: 0 })
//     createCercle(n, -45, { x: 0, y: stops[0] * 2, z: 0 }, { x: 0, y: 0, z: 0 })
//   } else {
//     // createCercle(n, 45, { x: 0, y: stops[1] * 2, z: 0 }, { x: 20, y: -10, z: 0 })
//     // createCercle(n, 45, { x: 0, y: stops[1] * 2, z: 0 }, { x: -20, y: -10, z: 0 })
//     // createCercle(n, -45, { x: 0, y: stops[1] * 2, z: 0 }, { x: 20, y: -10, z: 0 })
//     // createCercle(n, -45, { x: 0, y: stops[1] * 2, z: 0 }, { x: -20, y: -10, z: 0 })
//   }
...
function go (n = 0, stop = 0, i = 0, translate = { x: 0, y: 0 }) {
  // debugger
  ...
  createCercle(n, angle, { x: 0, y: stops[stop-1] * span || 0, z: 0 }, { x: translate.x, y: translate.y, z: 0 })
  ...
  if (n+1 < stops[stop]) {
    ...
    go(n+1, stop+1, i ? i : 1, { x: translate.x + ( 0 )*Math.cos(-angle) - ( posY )*Math.sin(-angle), y: translate.y + ( posY )*Math.cos(-angle) + ( 0 )*Math.sin(-angle) })
    go(n+1, stop+1, i ? -i : -1, { x: translate.x + ( 0 )*Math.cos(-angle) - ( posY )*Math.sin(-angle), y: translate.y + ( posY )*Math.cos(-angle) + ( 0 )*Math.sin(-angle) })
  }
}
go()
...
const fragmentShader = glsl(/* glsl */`
  ...
  void main () {
    ...
    gl_FragColor = vec4(color, alpha * opacity * visibility * 0.5 * oopa);
  }
`)
