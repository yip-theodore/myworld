...
// function createDesertGeometry () {
//   const geometry = new THREE.BufferGeometry()

  const uvs = new THREE.BufferAttribute(new Float32Array(length ** 3 * 3), 3)
  geometry.addAttribute('uv', uvs)
  const positions2 = new THREE.BufferAttribute(new Float32Array(length ** 3 * 3), 3)
  geometry.addAttribute('position2', positions2)
...
//   return geometry
// }
// const geometry = createDesertGeometry()
...
const vertexShader = glsl(/* glsl */`
  ...
  void main() {
    ...
    vec3 intermediatePosition = mix(position, position2, (time * 0.5 - floor(time * 0.5)));

    gl_Position = projectionMatrix * modelViewMatrix * vec4(intermediatePosition, 1.0);;
