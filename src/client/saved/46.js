...
function applyMatrix (position, angle) {
...
function applyRotation (position = { x: 0, y: 0, z: 0 }, angle = { x: 0, y: 0, z: 0 }, offset = { x: 0, y: 0, z: 0 }, splitNb) {
  ...
  const _position = {
    x: position.x,
    y: position.y,// - (splitNb * 10 * 5),
    z: position.z,
  }
  const _newPosition = applyMatrix(_position, {
    x: 0,
    y: -protection * 3,
    z: angle.z
  })
  const newPosition = applyMatrix(_newPosition, {
    x: 0,
    y: angle.y,
    z: 0
  })
...
function createCercle (radius = 10, pointsNb = 20, angle, offset, n, splitNb, nbls) {
  for (let i = 0; i < pointsNb; i++) {
    ...
    uvs.push(
      ( positions[i * 3 + 0] / radius + 1 ) / 2,
      n,
      ( positions[i * 3 + 2] / radius + 1 ) / 2,
    )
  }
}
...
function go (n = 1, angle = { x: 0, y: 0, z: 0 }, offset = { x: 0, y: 0, z: 0 }, splitNb = 0, nbls = 0, chance = 1) {
  if (protection++ > 10000) return console.log('infinite')
  if (n > 200) return console.log('n>')
  if (splitNb > splitNbMax) return// console.log('splitNb>')

  const radius = clamp(10 - (splitNb*1.5), 0.1, 10)
  const pointsNb = clamp(7 - (splitNb*1), 1, 20)

  createCercle(radius, pointsNb, angle, offset, n, splitNb, nbls)
  
  const doSplit = (n-(nbls || 20)) > 20*chance
  
  if (!doSplit) {
    go(n+1, angle, offset, splitNb, nbls, chance)

  } else {
    const yR = angle.y + 90
    const newAngle1 = {
      x: 0,
      y: yR + random.range(-45, 46),
      z: (angle.z || 45)*1.1 + random.range(-10, 11),
    }
    const newAngle2 = {
      x: -0,
      y: yR + random.range(-45, 46),
    ...
    const newOffset = getLastCercleCenter(pointsNb)
    
    go(n+1, newAngle1, newOffset, splitNb+1, n, chance*0.8)
    go(n+1, newAngle2, newOffset, splitNb+1, n, chance*0.8)
  }
}
...
const vertexShader = glsl(/* glsl */`
  ...
  void main() {
    ...
    mat3 m = rotationMatrix(vec3(0.0, 1.0, 0.0), toRad(time * 100.0));
    vec3 rotatedPosition = position * m;

    gl_Position = projectionMatrix * modelViewMatrix * vec4(rotatedPosition, 1.0);
    gl_PointSize = 5000.0 / length(cameraPosition - rotatedPosition);
  }
`)
const fragmentShader = glsl(/* glsl */`
  ...
  void main () {
    ...
    vec3 color = vec3(rgb(236.0), rgb(96.0), rgb(95.0));
